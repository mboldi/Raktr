...
export class ExportImportComponent {
  ...
  private async importDevices(file: File) {
    ...
    try {
      ...
      this.notifyError('Nem sikerült beolvasni a fájlt! Ellenőrizd, hogy valódi .xlsx fájlt választottál-e.');
      return;
    }
    ...
    try {
      ...
      this.importing = false;
      this.notifyError('Nem sikerült létrehozni a hiányzó kategóriákat/tárolási helyeket/tulajdonosokat!');
      return;
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      try {
        ...
        this.importFailures.push({
          ...
          name: this.cellToString(rows[i]['name']) || this.cellToString(rows[i]['assetTag']) || `${i + 2}. sor`,
  ...
  private async ensureCategoriesLocationsOwnersExist(rows: Record<string, CellValue>[]): Promise<Map<string, number>> {
    ...
    const missingOwners = Array.from(referencedOwnerNames).filter(name => !ownerIdByName.has(name));

    await Promise.all(missingCategories.map(name => firstValueFrom(this.categoryService.addCategory(name))));
    await Promise.all(missingLocations.map(name => firstValueFrom(this.locationService.addLocation(name))));

    const createdOwners = await Promise.all(
      missingOwners.map(name => firstValueFrom(this.ownerService.addOwner(new OwnerCreateDto(name, false))))
    );
  ...
  private collectMissingNames(rows: Record<string, CellValue>[], key: string, existing: Set<string>): string[] {
  ...
  private async buildDeviceCreateDto(row: Record<string, CellValue>, ownerIdByName: Map<string, number>): Promise<DeviceCreateDto> {
