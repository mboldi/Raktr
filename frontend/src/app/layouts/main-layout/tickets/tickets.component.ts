...
export class TicketsComponent implements OnInit {
  ...
  private getSortComparator(
    active: string,
  ): ((a: TicketDetails, b: TicketDetails) => number) | null {
    switch (active) {
      case 'id':
  ...
  private matchesSearch(ticket: TicketDetails): boolean {
    ...
    return (
      (ticket.scannable?.name ?? '').toLowerCase().includes(search) ||
    ...
    return this.tickets.filter(
      (ticket) =>
        ...
        (excludedFacet === 'status' ||
          ...
          this.selectedStatuses.has(ticket.status)) &&
        (excludedFacet === 'severity' ||
  ...
  private updateVisibleFilterOptions() {
    const availableStatuses = new Set(
      this.ticketsMatchingExcept('status').map((ticket) => ticket.status),
    );
    this.visibleStatuses = this.statuses.filter(
      (status) => availableStatuses.has(status) || this.selectedStatuses.has(status),
    );

    const availableSeverities = new Set(
      this.ticketsMatchingExcept('severity').map((ticket) => ticket.severity),
    );
    this.visibleSeverities = this.severities.filter(
      (severity) => availableSeverities.has(severity) || this.selectedSeverities.has(severity),
    );
