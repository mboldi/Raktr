...
export class CategoryService {
  ...
  getCategories(): Observable<CategoryDetails[]> {
    return this.http.get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/categories`).pipe(
      map((categories) => {
  ...
  addCategory(name: string): Observable<CategoryDetails> {
    ...
    return this.http
      .post<Record<string, unknown>>(`${environment.apiUrl}/v1/categories`, newCategory)
      .pipe(map((json) => CategoryDetails.fromJson(json)));
  }
