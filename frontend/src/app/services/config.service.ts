...
export class ConfigService {
  ...
  getConfigs(): Observable<ConfigDetailsDto[]> {
    return this.http.get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/configs`).pipe(
      map((categories) => {
  ...
  updateConfig(key: string, valueUpdate: ConfigUpdateDto): Observable<ConfigDetailsDto> {
    return this.http
      .put<Record<string, unknown>>(`${environment.apiUrl}/v1/configs/${key}`, valueUpdate)
      .pipe(map((json) => ConfigDetailsDto.fromJson(json)));
  }
}
