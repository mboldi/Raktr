...
export class ContainerService {
  ...
  getContainers(includeDeleted = false): Observable<ContainerDetails[]> {
    ...
    return this.http
      .get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/containers`, { params })
      .pipe(
        map((containers) => {
          const containers_typed: ContainerDetails[] = [];

          containers.forEach((container) =>
            containers_typed.push(ContainerDetails.fromJson(container)),
          );
  ...
  getContainer(containerId: number): Observable<ContainerDetails> {
    return this.http
      .get<Record<string, unknown>>(`${environment.apiUrl}/v1/containers/${containerId}`)
      .pipe(map((container) => ContainerDetails.fromJson(container)));
  }
  ...
  createContainer(containerToCreate: ContainerCreateDto): Observable<ContainerDetails> {
    ...
    return this.http
      .post<Record<string, unknown>>(
        `${environment.apiUrl}/v1/containers`,
        containerToCreate.toJson(),
        { headers: headers },
      )
      .pipe(map((createdContainer) => ContainerDetails.fromJson(createdContainer)));
  }

  updateContainer(
    containerId: number,
    containerUpdate: ContainerUpdateDto,
  ): Observable<ContainerDetails> {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');

    return this.http
      .put<Record<string, unknown>>(
        `${environment.apiUrl}/v1/containers/${containerId}`,
        containerUpdate.toJson(),
        { headers: headers },
      )
      .pipe(map((updatedContainer) => ContainerDetails.fromJson(updatedContainer)));
  }

  // returns whether the delete request was successful
  deleteContainer(containerId: number): Observable<boolean> {
    return this.http
      .delete(`${environment.apiUrl}/v1/containers/${containerId}`, { observe: 'response' })
      .pipe(
  ...
  restoreContainer(containerId: number): Observable<boolean> {
    return this.http
      .post<Record<string, unknown>>(
        `${environment.apiUrl}/v1/containers/${containerId}/restore`,
        '{}',
        { observe: 'response' },
      )
      .pipe(
  ...
  addDevicesToContainer(
    containerId: number,
    devicesToAdd: ContainerAddDevicesDto,
  ): Observable<ContainerDetails> {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');

    return this.http
      .post<Record<string, unknown>>(
        `${environment.apiUrl}/v1/containers/${containerId}/devices`,
        devicesToAdd.toJson(),
        { headers: headers },
      )
      .pipe(map((updatedContainer) => ContainerDetails.fromJson(updatedContainer)));
  ...
  updateDeviceInContainer(
    containerId: number,
    deviceId: number,
    itemUpdate: ContainerItemUpdateDto,
  ): Observable<ContainerDetails> {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');

    return this.http
      .put<Record<string, unknown>>(
        `${environment.apiUrl}/v1/containers/${containerId}/devices/${deviceId}`,
        itemUpdate.toJson(),
        { headers: headers },
      )
      .pipe(map((updatedContainer) => ContainerDetails.fromJson(updatedContainer)));
  ...
  getTicketsOfContainer(containerId: number): Observable<TicketDetails[]> {
    return this.http
      .get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/containers/${containerId}/tickets`)
      .pipe(
        map((tickets) => {
  ...
  getRentsOfContainer(containerId: number): Observable<RentDetails[]> {
    return this.http
      .get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/containers/${containerId}/rents`)
      .pipe(
        map((rents) => {
