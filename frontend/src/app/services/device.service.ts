...
export class DeviceService {
  ...
  getDevices(includeDeleted = false): Observable<DeviceDetails[]> {
    ...
    return this.http
      .get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/devices`, { params })
      .pipe(
        map((devices) => {
  ...
  getDevice(deviceId: number): Observable<DeviceDetails> {
    return this.http
      .get<Record<string, unknown>>(`${environment.apiUrl}/v1/devices/${deviceId}`)
      .pipe(
        map((device) => {
  ...
  updateDevice(deviceId: number, deviceUpdate: DeviceUpdateDto): Observable<DeviceDetails> {
    ...
    return this.http
      .put<Record<string, unknown>>(
        `${environment.apiUrl}/v1/devices/${deviceId}`,
        deviceUpdate.toJson(),
        { headers: headers },
      )
      .pipe(map((updatedDevice) => DeviceDetails.fromJson(updatedDevice)));
  }

  // returns whether the delete request was successful
  deleteDevice(deviceId: number): Observable<boolean> {
    return this.http
      .delete(`${environment.apiUrl}/v1/devices/${deviceId}`, { observe: 'response' })
      .pipe(
  ...
  createDevice(deviceToCreate: DeviceCreateDto): Observable<DeviceDetails> {
    ...
    return this.http
      .post<Record<string, unknown>>(`${environment.apiUrl}/v1/devices`, deviceToCreate.toJson(), {
        headers: headers,
      })
      .pipe(map((createdDevice) => DeviceDetails.fromJson(createdDevice)));
  }
  ...
  restoreDevice(deviceIdToRestore: number): Observable<boolean> {
    return this.http
      .post<Record<string, unknown>>(
        `${environment.apiUrl}/v1/devices/${deviceIdToRestore}/restore`,
        '{}',
        { observe: 'response' },
      )
      .pipe(
  ...
  getTicketsOfDevice(deviceId: number): Observable<TicketDetails[]> {
    return this.http
      .get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/devices/${deviceId}/tickets`)
      .pipe(
        map((tickets) => {
  ...
  getRentsOfDevice(deviceId: number): Observable<RentDetails[]> {
    return this.http
      .get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/devices/${deviceId}/rents`)
      .pipe(
        map((tickets) => {
