...
export class LocationService {
  ...
  getLocations(): Observable<LocationDetails[]> {
    return this.http.get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/locations`).pipe(
      map((locations) => {
  ...
  addLocation(name: string): Observable<LocationDetails> {
    ...
    return this.http
      .post<Record<string, unknown>>(`${environment.apiUrl}/v1/locations`, newLocation)
      .pipe(map((json) => LocationDetails.fromJson(json)));
  }
