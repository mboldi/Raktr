...
export class OwnerService {
  ...
  getOwners(): Observable<OwnerDetailsDto[]> {
    return this.http.get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/owners`).pipe(
      map((owners) => {
  ...
  addOwner(newOwner: OwnerCreateDto): Observable<OwnerDetailsDto> {
    ...
    return this.http
      .post<Record<string, unknown>>(`${environment.apiUrl}/v1/owners`, newOwner, {
        headers: headers,
      })
      .pipe(map((createdOwner) => OwnerDetailsDto.fromJson(createdOwner)));
  }
  ...
  updateOwner(ownerId: number, ownerUpdate: OwnerUpdateDto): Observable<OwnerDetailsDto> {
    ...
    return this.http
      .put<Record<string, unknown>>(`${environment.apiUrl}/v1/owners/${ownerId}`, ownerUpdate, {
        headers: headers,
      })
      .pipe(map((updatedOwner) => OwnerDetailsDto.fromJson(updatedOwner)));
  }
