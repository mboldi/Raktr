...
export class RentService {
  ...
  getRentCount(): Observable<number> {
    return this.http.get<Record<string, unknown>[]>(`${environment.apiUrl}/v1/rents`).pipe(
      map((rents) => {
        return rents.length;
      }),
    );
  ...
  getRent(rentId: number): Observable<RentDetails> {
    return this.http
      .get<Record<string, unknown>>(`${environment.apiUrl}/v1/rents/${rentId}`)
      .pipe(map((rent) => RentDetails.fromJson(rent)));
  }
  ...
  createRent(rentToCreate: RentCreateDto): Observable<RentDetails> {
    ...
    return this.http
      .post<Record<string, unknown>>(`${environment.apiUrl}/v1/rents`, rentToCreate.toJson(), {
        headers: headers,
      })
      .pipe(map((createdRent) => RentDetails.fromJson(createdRent)));
  }
  ...
  updateRent(rentId: number, rentUpdate: RentUpdateDto): Observable<RentDetails> {
    ...
    return this.http
      .put<Record<string, unknown>>(
        `${environment.apiUrl}/v1/rents/${rentId}`,
        rentUpdate.toJson(),
        { headers: headers },
      )
      .pipe(map((updatedRent) => RentDetails.fromJson(updatedRent)));
  }
  ...
  addRentItem(rentId: number, itemToAdd: RentItemCreateDto): Observable<RentItemDetailsDto> {
    ...
    return this.http
      .post<Record<string, unknown>>(`${environment.apiUrl}/v1/rents/${rentId}/items`, itemToAdd, {
        headers: headers,
      })
      .pipe(map((createdItem) => RentItemDetailsDto.fromJson(createdItem)));
  }

  updateRentItem(
    rentId: number,
    rentItemId: number,
    itemUpdate: RentItemUpdateDto,
  ): Observable<RentItemDetailsDto> {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');

    return this.http
      .put<Record<string, unknown>>(
        `${environment.apiUrl}/v1/rents/${rentId}/items/${rentItemId}`,
        itemUpdate.toJson(),
        { headers: headers },
      )
      .pipe(map((updatedItem) => RentItemDetailsDto.fromJson(updatedItem)));
  }
  ...
  addComment(rentId: number, comment: CommentCreateDto): Observable<CommentDetailsDto> {
    ...
    return this.http
      .post<Record<string, unknown>>(`${environment.apiUrl}/v1/rents/${rentId}/comments`, comment, {
        headers: headers,
      })
      .pipe(map((createdComment) => CommentDetailsDto.fromJson(createdComment)));
  }

  getRentPdf(rentId: number, pdfRequest: RentPdfCreateDto): Observable<Blob> {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');

    return this.http.post(`${environment.apiUrl}/v1/rents/${rentId}/pdf`, pdfRequest, {
      headers: headers,
      responseType: 'blob',
    });
  }
