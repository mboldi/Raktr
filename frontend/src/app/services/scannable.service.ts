...
export class ScannableService {
  ...
  getScannablesCount(): Observable<number> {
    return this.http.get<number>(`${environment.apiUrl}/v1/scannables/count`).pipe(
      map((count) => {
        return count;
      }),
    );
  ...
  isBarcodeTaken(barcodeToCheck: string): Observable<boolean> {
    return this.http
      .head(`${environment.apiUrl}/v1/scannables/barcode/${barcodeToCheck}`, {
        observe: 'response',
      })
      .pipe(
  ...
  isAssetTagTaken(assetTagToCheck: string): Observable<boolean> {
    return this.http
      .head(`${environment.apiUrl}/v1/scannables/asset-tag/${assetTagToCheck}`, {
        observe: 'response',
      })
      .pipe(
        map((response) => response.status === 200),
  ...
  getByBarcode(barcode: string): Observable<ScannableDetailsDto> {
    return this.http
      .get<Record<string, unknown>>(`${environment.apiUrl}/v1/scannables/barcode/${barcode}`)
      .pipe(
        map((device) => {
