...
export class TicketService {
  ...
  createTicket(ticketToCreate: TicketCreateDto): Observable<TicketDetails> {
    ...
    return this.http
      .post<Record<string, unknown>>(`${environment.apiUrl}/v1/tickets`, ticketToCreate.toJson(), {
        headers: headers,
      })
      .pipe(map((createdTicket) => TicketDetails.fromJson(createdTicket)));
  }
  ...
  updateTicket(ticketId: number, ticketUpdate: TicketUpdateDto): Observable<TicketDetails> {
    ...
    return this.http
      .put<Record<string, unknown>>(
        `${environment.apiUrl}/v1/tickets/${ticketId}`,
        ticketUpdate.toJson(),
        { headers: headers },
      )
      .pipe(map((updatedTicket) => TicketDetails.fromJson(updatedTicket)));
  }
  ...
  addComment(ticketId: number, comment: CommentCreateDto): Observable<CommentDetailsDto> {
    ...
    return this.http
      .post<Record<string, unknown>>(
        `${environment.apiUrl}/v1/tickets/${ticketId}/comments`,
        comment,
        { headers: headers },
      )
